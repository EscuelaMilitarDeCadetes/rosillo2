...
const EstadisticaProyectosEnDesarrollo = () => {
  ...
  const header = (
    <div className="d-flex justify-content-between align-items-center">
      <h5 className="m-0">Estadísticas de Proyectos en Desarrollo</h5>
  ...
  const chartData = {
    labels: ['En Inicio', 'En Ejecución', 'Finalizados'],
    datasets: [
      {
        data: [
          filteredProjects.filter(p => p.estado === 'En Inicio').length,
          filteredProjects.filter(p => p.estado === 'En Ejecución').length,
          filteredProjects.filter(p => p.estado === 'Finalizado').length,
        ],
...
export default EstadisticaProyectosEnDesarrollo;
