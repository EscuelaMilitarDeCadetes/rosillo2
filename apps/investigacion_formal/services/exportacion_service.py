...
ENCABEZADOS_TIPOS_PRODUCTO = ["Tipo de Producto", "Aplica"]
...
class ExportacionService:
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _armar_fila_tipo_producto(tipo):
        return [
            tipo.tipo_producto,
            "Sí" if tipo.aplica else "No",
        ]

    @staticmethod
    def exportar_excel_tipos_producto(queryset):
        wb = Workbook()
        sheet = wb.active
        sheet.title = "Tipos de Producto"
        sheet.append(ENCABEZADOS_TIPOS_PRODUCTO)
        for celda in sheet[1]:
            celda.font = XlFont(bold=True, color="FFFFFF")
            celda.fill = PatternFill(start_color="404040", end_color="404040", fill_type="solid")
        for registro in queryset:
            sheet.append(ExportacionService._armar_fila_tipo_producto(registro))
        for columna in sheet.columns:
            longitud = max(len(str(c.value)) if c.value is not None else 0 for c in columna)
            sheet.column_dimensions[columna[0].column_letter].width = min(longitud + 2, 60)
        buffer = BytesIO()
    ...
    @staticmethod
    def exportar_pdf_tipos_producto(queryset):
        buffer = BytesIO()
        documento = SimpleDocTemplate(
            buffer,
            pagesize=A4,
        ...
        estilos = getSampleStyleSheet()
        titulo_estilo = ParagraphStyle(
            "TituloExportTiposProducto", parent=estilos["Heading1"], alignment=1, fontSize=16,
        )
        celda_estilo = ParagraphStyle("CeldaExportTiposProducto", parent=estilos["Normal"], fontSize=9, leading=11)
        encabezado_estilo = ParagraphStyle(
            "EncabezadoExportTiposProducto", parent=estilos["Normal"], fontSize=9.5, leading=11,
            textColor=colors.white,
        )
        datos = [[Paragraph(h, encabezado_estilo) for h in ENCABEZADOS_TIPOS_PRODUCTO]]
        for registro in queryset:
            fila = ExportacionService._armar_fila_tipo_producto(registro)
            datos.append([Paragraph(str(valor), celda_estilo) for valor in fila])
        anchos_mm = [(A4[0] - 30 * mm) * 0.75, (A4[0] - 30 * mm) * 0.25]
        tabla = Table(datos, colWidths=anchos_mm, repeatRows=1)
        tabla.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor("#404040")),
            ('GRID', (0, 0), (-1, -1), 0.4, colors.grey),
        ...
        documento.build([
            Paragraph("Tipos de Producto", titulo_estilo),
            Spacer(1, 8 * mm),
            tabla,
        ])
        buffer.seek(0)
