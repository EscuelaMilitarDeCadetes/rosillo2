...
class TipoRubroServiceTests(InvestigacionFormalFixturesMixin, TestCase):
    ...
    def test_crear_tipo_rubro_exitoso(self):
        rubro = TipoRubroService.crear(nombre_rubro='Personal', ejecutor=self.ejecutor)
        self.assertEqual(rubro.nombre_rubro, 'Personal')

    def test_crear_tipo_rubro_nombre_duplicado_falla(self):
        TipoRubroService.crear(nombre_rubro='Equipos', ejecutor=self.ejecutor)
        with self.assertRaises(ValidationError):
            TipoRubroService.crear(nombre_rubro='equipos', ejecutor=self.ejecutor)

    def test_crear_tipo_rubro_nombre_vacio_falla(self):
        with self.assertRaises(ValidationError):
            TipoRubroService.crear(nombre_rubro='   ', ejecutor=self.ejecutor)

    def test_actualizar_tipo_rubro_exitoso(self):
        rubro = TipoRubroService.crear(nombre_rubro='Viáticos', ejecutor=self.ejecutor)
        actualizado = TipoRubroService.actualizar(
            tipo_rubro_id=rubro.pk, nombre_rubro='Viáticos y Transporte', ejecutor=self.ejecutor,
        )
    ...
    def test_actualizar_tipo_rubro_nombre_duplicado_con_otro_falla(self):
        TipoRubroService.crear(nombre_rubro='Materiales', ejecutor=self.ejecutor)
        rubro2 = TipoRubroService.crear(nombre_rubro='Software', ejecutor=self.ejecutor)
        with self.assertRaises(ValidationError):
            TipoRubroService.actualizar(
                tipo_rubro_id=rubro2.pk, nombre_rubro='Materiales', ejecutor=self.ejecutor,
            )

    def test_listar_tipos_rubro(self):
        TipoRubroService.crear(nombre_rubro='Rubro 1', ejecutor=self.ejecutor)
        TipoRubroService.crear(nombre_rubro='Rubro 2', ejecutor=self.ejecutor)
